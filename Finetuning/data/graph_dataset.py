...
class DockingDataset(Dataset):
    def __init__(self, dir_path, name_list=None, poc_max_len=None,
                 normalize_labels=False, label_min=None, label_max=None):  # 修改参数
        self.dir_path = dir_path
        self.name_list = os.listdir(dir_path)
        ...
        label_file = 'ASRSMA/Dataset/Finetuning/input_label/pdbbind_NL_labels.csv'
        labels = pd.read_csv(label_file)
        self.labels = labels.set_index('pdb')['label'].to_dict()


        if self.normalize_labels:
            if label_min is None or label_max is None:
                # 从当前数据集的name_list中提取所有标签
                all_labels = [self.labels[name.split('_')[0]] for name in self.name_list]
                self.label_min = np.min(all_labels)
                self.label_max = np.max(all_labels)
                print("self.label_min: ", self.label_min)
                print("self.label_max: ", self.label_max)
            else:
    ...
    def __getitem__(self, idx):
        ...
        name = data_name.split('_')[0]
        # print(f"idx: {idx},  data_name: {data_name}")
        file_p =  os.path.join(self.dir_path, f'{data_name}')
        file_path = open(file_p, 'rb')
        ...
        # print(f"poc_graph.x {poc_graph.x}")
        ...
        if self.normalize_labels:
            ...
            infos['label'] = torch.tensor(label, dtype=torch.float32)
...
def getpkl(file_p,poc_max_len):
        file_path = open(file_p, 'rb')
        data = pickle.load(file_path)
        file_path.close()
        data = sample_conf(data)
        data = crop_pocket(data, poc_max_len)
        data = Normalize_data(data)
        lig_chem_feats = torch.from_numpy(data['lig_feats'])
        poc_chem_feats = torch.from_numpy(data['poc_feats'])
        poc_src = np.hstack([data['poc_bonds'][0], data['poc_bonds'][1]])[np.newaxis, :]
