...
class MolTransformer(nn.Module):
    ...
    def forward(self, batch_data, drop_prob=None):
        node_embed = self.atom_feature(batch_data)

        if self.training and getattr(self.args, "use_random_mask", True):
            if drop_prob is not None and drop_prob > 0:
                node_embed[:, 1:, :] = random_drop_atoms(node_embed[:, 1:, :], drop_prob=drop_prob)
